...
def main():
    ...
    if exists("cast_checkpoint.json"):
        with open("cast_checkpoint.json", "r") as checkpoint_file:
            line = checkpoint_file.readline()
            last_processed_line = json.loads(line)["last_processed_line"]

    cast_metadata = [] # movie_id, cast_id, role
    with open("movie_ids_04_12_2024.json", encoding='utf-8') as movie_ids_file:
        for line_number, line in enumerate(tqdm(movie_ids_file, total=916746)):
            ...
            if "cast" not in response.json():
                continue
            cast = response.json()["cast"]

            for member in cast:
                cast_metadata.append([id, member["id"], "actor"])

            if line_number % 5000 == 0: # checkpoint: dump batch of cast data into csv file
                with open("cast_data.csv", "a") as output_file:
                    csv_writer = csv.writer(output_file, delimiter=",", quotechar='"')
                    for entry in cast_metadata:
                        csv_writer.writerow(entry)

                cast_metadata.clear()

                with open("cast_checkpoint.json", "w") as checkpoint_file:
